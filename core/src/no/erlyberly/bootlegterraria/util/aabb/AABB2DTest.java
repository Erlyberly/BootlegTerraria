...
class AABB2DTest {
    ...
    void overlap() {
        ...
        final AABB2D tst1 = new AABB2D(0, 10, 0, 10);
        final AABB2D tst2 = new AABB2D(5, 15, 5, 15);
        final AABB2D tst3 = new AABB2D(15, 18, 5, 15);
    ...
    @Test
    void iterator() {
        final AABB2D tst1 = new AABB2D(0, 2, 0, 2);

        final ArrayList<Vector2Int> act = new ArrayList<>();

        for (final Vector2Int v : tst1) {
            act.add(v);
        }
        Vector2Int[] actv = new Vector2Int[act.size()];
        actv = act.toArray(actv);
        final Vector2Int[] exp =
